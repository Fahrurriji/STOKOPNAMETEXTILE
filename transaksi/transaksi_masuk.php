...
$data = $conn->query("SELECT tm.*, p.nama_pemasok, a.nama_lengkap as nama_admin
...
<div class="card">
  <div class="card-header">
    ...
    <div><h3>Transaksi Barang Masuk</h3><p>Pembelian & penerimaan kain dari pemasok</p></div>
    <a href="tambah_masuk.php" class="btn btn-success btn-sm"><i class="fas fa-plus"></i> Tambah Transaksi</a>
  </div>
  <div class="page-toolbar">
    <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      <div class="form-group" style="margin:0"><label style="font-size:11px;margin-bottom:3px;display:block">Dari Tanggal</label>
        <input type="date" name="tgl_awal" class="form-control no-icon" style="padding:8px 12px;width:160px" value="<?= $tgl_awal ?>">
      </div>
      <div class="form-group" style="margin:0"><label style="font-size:11px;margin-bottom:3px;display:block">Sampai Tanggal</label>
  ...
  <div class="table-wrap">
    <table class="table">
      <thead><tr><th>No</th><th>No. Transaksi</th><th>Tanggal</th><th>Pemasok</th><th>Total Item</th><th>Total Nilai</th><th>Admin</th><th>Aksi</th></tr></thead>
      <tbody>
      <?php if ($data->num_rows === 0): ?>
        <tr><td colspan="8"><div class="empty-state"><div class="es-icon">📦</div><h4>Tidak ada transaksi</h4><p>Klik "Tambah Transaksi" untuk mencatat pembelian baru</p></div></td></tr>
      <?php else: $no=1; $total=0; while ($r = $data->fetch_assoc()): $total += $r['total_nilai']; ?>
        <tr>
          <td><?= $no++ ?></td>
          ...
          <td><div class="action-btns">
            <a href="detail_masuk.php?id=<?= $r['id'] ?>" class="btn btn-primary btn-sm" data-tooltip="Detail"><i class="fas fa-eye"></i></a>
            <a href="#" onclick="confirmDelete('hapus_masuk.php?id=<?= $r['id'] ?>','<?= htmlspecialchars($r['no_transaksi']) ?>')" class="btn btn-danger btn-sm" data-tooltip="Hapus"><i class="fas fa-trash"></i></a>
          </div></td>
        </tr>
      <?php endwhile; ?>
        <tr style="background:#f8fafc;font-weight:700">
          <td colspan="5" style="text-align:right">TOTAL PERIODE:</td>
          <td style="color:var(--success)"><?= rupiah($total) ?></td>
          <td colspan="2"></td>
        </tr>
      <?php endif; ?>
      </tbody>
